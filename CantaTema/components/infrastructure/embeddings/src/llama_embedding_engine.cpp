...
std::vector<float> LlamaEmbeddingEngine::generate_embedding(const std::string& text) {
    if (!m_context) {
    ...
    int n_tokens = m_context->tokenize(text, tokens);
    if (n_tokens < 0) {
...
std::vector<std::vector<float>> LlamaEmbeddingEngine::generate_embeddings_batch(const std::vector<std::string>& texts) {
    std::vector<std::vector<float>> results;
    results.reserve(texts.size());
    for (const auto& text : texts) {
        results.push_back(generate_embedding(text));
    }
    return results;
}
