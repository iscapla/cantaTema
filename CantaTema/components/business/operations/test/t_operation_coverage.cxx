...
TEST_F(OperationCoverageTest, UserConfigurationCustomExecutionWeights) {
    auto practice = std::make_shared<PracticeEvent>();
    practice->set_id(1);
    practice->set_subject_id(10);
    practice->set_filepath(test_audio_file);
    ...
    sim.pdf_chunk_index = 0;
    sim.best_transcript_chunk_index = 0;
    sim.similarity_score = 0.90f;
    sim.is_mentioned = true;
    mock_matches.push_back(sim);

    // Verify per-execution search options match custom UserConfiguration settings
    EXPECT_CALL(*mock_similarity, search_pdf_matches_advanced(_, _, _, _, ::testing::AllOf(
        ::testing::Field(&SimilaritySearchOptions::similarity_threshold, ::testing::FloatEq(0.88f)),
        ::testing::Field(&SimilaritySearchOptions::numeric_boost, ::testing::FloatEq(0.25f))
    )))
    .WillOnce(Return(mock_matches));

    // Verify DB snapshot contains custom config string
    EXPECT_CALL(*mock_db, save_coverage_analysis_execution(1, _, _, _, _, _, _, _, _, _, ::testing::HasSubstr("0.88"), _))
        .WillOnce(Return(RST_OK));

    UserConfiguration custom_cfg;
    custom_cfg.reference_extraction.importance_weight_bold = 3.0f;
    custom_cfg.comparison.similarity_threshold = 0.88f;
    custom_cfg.comparison.numeric_boost = 0.25f;

    std::string exec_id;
    rst_code_e res = coverage_op->analyze_practice_coverage(user, 1, custom_cfg, exec_id);
